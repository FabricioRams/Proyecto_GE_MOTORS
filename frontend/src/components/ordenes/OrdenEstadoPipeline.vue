<template>
  <div class="w-full">
    ...
    <!-- Block reason for repair state (RN-04) -->
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          <p class="text-sm text-yellow-700">
            <strong>Bloqueado:</strong> No se puede avanzar a Reparación hasta que el presupuesto sea aprobado por el cliente.
          </p>
        </div>
      </div>
    </div>
...
const props = defineProps({
  estadoActual: {
    type: String,
    required: true,
    validator: (v) => ['diagnostico', 'reparacion', 'esperando_repuesto', 'control_calidad', 'entregado'].includes(v)
  },
  presupuestoAprobado: {
    type: Boolean,
    default: false
...
const estados = computed(() => {
  const result = ordenEstados.map((key, index) => {
    ...
    if (esActual) {
      ...
      if (key === 'reparacion' && !props.presupuestoAprobado) {
        clickable = false
      } else {
        clickable = true
      }
...
const mostrarBloqueoReparacion = computed(() => {
  return props.estadoActual === 'presupuesto' && !props.presupuestoAprobado
})
